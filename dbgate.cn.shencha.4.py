...
def updateDB(db_ip,db_port,db_username,db_password,db_name):
		...
		command = '''update DBgate_Configer set db_accesscheck4 =1 where db_ip = "%s" and db_port = "%s";'''%(db_ip,db_port)
		print command
		cur = conn.cursor()
		cur.execute(command)
		conn.commit()
...
#cur.execute(command)
#conn.commit()
command = ''' select db_ip, db_port, db_username, db_password, db_name from DBgate_Configer where db_accesscheck4 = 0;'''
#print command
cur.execute(command)
